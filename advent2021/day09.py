...
def get_higher_neighbors(grid, x, y) -> list[tuple[int, int]]:
    """Get the coordinates of the neighborign cells that are also greater."""
    cur = grid[x][y]
    ret = []
    if x > 0:
        if grid[x - 1][y] > cur:
    ...
    for mx, my in local_minima(grid)[1]:
        ...
        while len(edge) > 0:
            ...
            for (x, y) in edge:
                ...
                for nx, ny in get_higher_neighbors(grid, x, y):
                    # if this neighbor was already explored, skip it
                    if (nx, ny) in basin or (nx, ny) in edge:
                        continue
                    # also ignore 9, as asked by the problem
                    if grid[nx][ny] == 9:
                        continue
